...
function openEditStopModal(dayIdx,stopIdx){
  ...
  document.getElementById('modal-overlay').classList.add('open');
  try{_fitFieldRows();}catch(e){}
  // Was a wrapper in trip-extras.js that reassigned window.openEditStopModal.
  // NOTE: it must NOT touch f-endtime — this function already fills it, and the
  // field is a native <input type="time"> that rejects a 12-hour string.
  const _au=document.getElementById('f-audiourl');if(_au)_au.value=(s&&s.audioUrl)||'';
...
// true / false / null(unknown) — is this place open at that many minutes past
// midnight? `edge` decides what the closing minute itself means: an ARRIVAL at
// closing time is too late, but LEAVING at closing time is exactly right.
function _isOpenAt(hoursLine,mins,edge){
  const w=_hoursWindows(hoursLine);
  if(w===null||mins==null)return null;
  if(!w.length)return false;                                 // closed today
  const inWin=(m,x)=>m>=x.open&&(edge==='leaving'?m<=x.close:m<x.close);
  return w.some(x=>inWin(mins,x)||inWin(mins+1440,x));
}
// How far outside the hours a time falls, in minutes (0 when inside).
function _minsOutsideHours(hoursLine,mins){
  const w=_hoursWindows(hoursLine);
  if(w===null||mins==null||!w.length)return null;
  let best=Infinity;
  w.forEach(x=>{
    if(mins>=x.open&&mins<=x.close)best=0;
    else best=Math.min(best,mins<x.open?(x.open-mins):(mins-x.close));
  });
  return best===Infinity?null:best;
}
// Turning up a couple of minutes before the doors open is not a planning error.
...
function _applyGradeCap(letter,blockedCount,totalStops){
  ...
  if(li<0)return letter;
  return li<ci?cap:letter;                        // never raise, only cap
}
...
/* ============================================================
   ...
   ============================================================ */
// The attachment rules that used to sit on PLAN_CHAT_SYSTEM are now appended to
// _PLAN_SYS below — PLAN_CHAT_SYSTEM belonged to the dead _planCallAI and was
// never sent to the model, so those instructions had no effect at all.
...
// _planCallAI lives further down, with the itinerary-changes machinery it
// belongs to. There used to be TWO definitions: this one, and a replacement
// installed by trip-extras.js at load. Only the replacement ever ran, so
// edits made here silently did nothing.
...
/* ============================================================
   MERGED FROM trip-extras.js (v190)
   That file was a second <script> that reassigned window.saveStop,
   window.openEditStopModal and window._planCallAI at load time. A fix
   applied to any of those in trip.js could be undone by the wrapper, and
   two of the three definitions were dead code nobody could see was dead.
   Everything now lives here, once.
   ============================================================ */
// [merged into trip.js] _esc removed: trip.js already has _escHtml
...
function augmentCards(){
  ...
  document.querySelectorAll('.stop-card').forEach(card => {
    ...
    if(stop.endTime && !timeEl.querySelector('.card-endtime')){
      ...
      endEl.innerHTML = '&#8594; ' + _escHtml(stop.endTime) + (dTz ? ' <span style="font-size:var(--text-xs);font-weight:700;letter-spacing:0.10em;color:var(--river);opacity:0.85">'+_escHtml(dTz.abbr)+'</span>' : '');
...
function _startObserver(){
  if(typeof MutationObserver==='undefined')return;   // no DOM (unit-test vm): nothing to observe
  _watchPlanChatModal();
  _injectFormField();
  augmentCards();
  _patchEndOfTrip();
...
// [removed] _syncOvernightArrivals_DISABLED: dead since a stop that spans
// midnight became ONE event. Its only helper, _parseMinutes, went with it
// (trip.js's _parseTimeMins is the single time parser).
...
function _renderChangePanel(jsonStr){
  ...
  changes.forEach((c,i) => {
    rows += '<div style="padding:var(--space-1) 0;font-size:var(--text-sm);color:var(--ink-soft);border-bottom:1px solid rgba(46,125,82,0.12)">'+
      '<strong style="color:var(--pine)">'+(i+1)+'.</strong> '+_escHtml(_cleanChangeText(c.description)||_humanizeChange(c))+'</div>';
  });
...
// Wired inside _startObserver with the other observers. It used to run at
// parse time, which made merely LOADING this file depend on a live DOM.
function _watchPlanChatModal(){
  if(typeof MutationObserver==='undefined')return;
  const m=document.getElementById('plan-chat-modal');
  if(!m)return;
  new MutationObserver(ms=>ms.forEach(x=>{ if(x.target.classList.contains('open')) _injectPlanChatBtn(); }))
